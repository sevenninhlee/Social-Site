		...
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			...
			<div class="row">
				<div class="col-md-6 col-sm-12">
					...
					<ul class="list-inline list_all">
						<li>
	                        <select class="select_list" id="select_list_blogs">
								...
								<option value="delete_blogs" act="deleteBlogArticle">Delete</option>
								<option value="activate_blogs" act="changeStatusManyBlogArticle">Activate</option>
								<option value="deactivate_blogs" act="changeStatusManyBlogArticle">Deactivate</option>
	                        </select>
					...
					<ul class="list-inline list_all">
						<li>
							<select class="select_list" id="select_list_blogs_author">
							...
							<select class="select_list" id="select_list_blogs_category">
							...
							<select class="select_list" id="select_list_blogs_featured_blog">
								<option value="all" <?php if(isset($app['prs']['featured_blog']) && $app['prs']['featured_blog']=='all-featured_blog') echo 'selected' ?>>All featured blogs</option>
								<option value="1" <?php if(isset($app['prs']['featured_blog']) && $app['prs']['featured_blog']=='1') echo 'selected' ?>>Yes</option>
								<option value="0" <?php if(isset($app['prs']['featured_blog']) && $app['prs']['featured_blog']=='0') echo 'selected' ?>>No</option>
							</select>
							<select class="select_list" id="select_list_blogs_status">
							...
							<button class="btn-custom btn btn-apply" id='btn_filter_blogs_table'>Filter</button></li>
						</li>
					</ul>
				</div>
				<div class="col-md-6 col-sm-12">
					<form id="form-blogs-search">
			...
			<table class="table dataTable" id="blog_table" controller="blogs" role="grid" aria-describedby="example1_info">
				<tr>
					...
					<th class="text-center">Featured Blog</th>
				...
				<tbody class="tbl_body records">
					<?php if(count($this->records['data'])) { ?>
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act" style='max-width:250px;'>
								<div class="checkbox">
									...
									<label for="<?php echo("checkbox-".$record['id']);?>">
										<a target="_blank" href="<?php echo RootURL."blogs/view/".$record['slug']?>" id="viewUser<?=$record['id'];?>">
								...
								<ul class="list-inline">
									<li>
										<a target="_blank" class='btn-delete-table' href="<?php echo RootURL."blogs/view/".$record['slug']?>" id="viewUser<?=$record['id'];?>">
											View
										</a>	
									</li>
									<li><a class='btn-delete-table' href="<?php echo (vendor_app_util::url(["ctl"=>"blogs", "act"=>"edit/".$record['id']])) ?>" id="<?php echo("edit".$record['id']);?>" type="button" >Edit</a></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteBlogArticle">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow">
								<p class="andrew">
									<?php if($record['featured_my_blog']=='1'){ echo 'Yes'; } else if($record['featured_my_blog']=='2') { echo 'Private'; } else { echo 'No'; } ?>
								</p>
							</td>

							<td class="webShow" id="<?php echo("featured_image".$record['id']);?>">
								<p class="andrew">
									<a target="_blank" href="<?php echo RootURL."blogs/view/".$record['slug']?>" id="featured_imageViewUser<?=$record['id'];?>">
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								...
								?>
								<button type="button" class="btn btn-primary change_status_blogs" alt="<?php echo($record['id']);?>,0,changeStatusBlogArticle" >Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_blogs" alt="<?php echo($record['id']);?>,1,changeStatusBlogArticle">Inactive</button>
								<?php 
									endif;
								?>
							</td>
