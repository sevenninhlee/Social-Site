...
<div class="col-md-10 col-sm-9 pad0">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">Books categories</p>
					...
					<ul class="list-inline list_all">
						<li>
							<select class="select_list" id="select_list_books">
								...
								<option value="delete_books" act="deleteBookCategory">Delete</option>
								<option value="activate_books" act="changeStatusManyBookCategory">Activate</option>
								<option value="deactivate_books" act="changeStatusManyBookCategory">Deactivate</option>
							</select>
							<button class="btn-custom btn btn-apply" id='btn_apply_books_table'>Apply</button>
						</li>
						<li>
							<select class="select_list" id="select_list_books_categories_status">
							...
							<button class="btn-custom btn btn-apply" id='btn_filter_books_categories_table'>Filter</button></li>
						</li>
						<li>
							<form id="form-books-categories-add">
								<label>
						...
						<li>
							<p id='form-book-categories-err-name' class='text-danger text-center'></p>
						</li>
					</ul>
				</div>
				<div class="col-md-6 col-sm-12">
					<form id="form-books-categories-search">
			...
			<div class="table-responsive">
				<table class="table table-striped dataTable" id="book_table" controller="books" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
					<tr>
								...
								<tr role="row" id="row<?= $record['id']; ?>">
									...
									<td id="<?php echo ("checkbox" . $record['id']); ?>" class="checkboxRecord btn-act">
										...
										<ul class="list-inline features">
											<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-books-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
											<li><button id="del<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteBookCategory">Delete</button></li>
										</ul>
									</td>
									...
									<td class="tabletShow" id="<?php echo ("status" . $record['id']); ?>">
										...
										if ($record['status'] == 1) :
										?>
											<button type="button" class="btn btn-primary change_status_books" alt="<?php echo ($record['id']); ?>,0,changeStatusBookCategory">Active</button>
										<?php
										else :
										?>
											<button type="button" class="btn btn-warning change_status_books" alt="<?php echo ($record['id']); ?>,1,changeStatusBookCategory">Inactive</button>
										<?php
										endif;
										?>
									</td>
