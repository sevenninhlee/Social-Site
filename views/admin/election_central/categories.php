...
<div class="col-md-10 col-sm-9 pad0" id="page_election_central">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">Election centrals categories</p>
				...
				<ul class="list-inline list_all">
					<li>
						<select class="select_list" id="select_list_election_central">
							...
							<option value="delete_election_central" act="deleteElectionCentralCategory">Delete</option>
							<option value="activate_election_central" act="changeStatusManyElectionCentralCategory">Activate</option>
							<option value="deactivate_election_central" act="changeStatusManyElectionCentralCategory">Deactivate</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_apply_election_central_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_election_central_categories_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_election_central_categories_table'>Filter</button></li>
					</li>
					<li>
                        <form id="form-election_central-categories-add">
                            <label>
			...
			<div class="col-md-6 col-sm-12">
				<form id="form-election_central-categories-search">
			...
			<table class="table table-striped dataTable" id="blog_table" controller="election_central" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
				...
				<tbody class="tbl_body records">
					<?php if(count($this->records['data'])) { ?>
						<?php $i=1+($this->records['curp']-1)*$this->records['nopp']; foreach ($this->records['data'] as $key => $record) { ?>
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								...
								<ul class="list-inline features">
									<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-election_central-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteElectionCentralCategory">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								<?php 
									if($record['status'] == 1 ):
								?>
								<button type="button" class="btn btn-primary change_status_election_central" alt="<?php echo($record['id']);?>,0,changeStatusElectionCentralCategory">Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_election_central" alt="<?php echo($record['id']);?>,1,changeStatusElectionCentralCategory">Inactive</button>
								<?php 
									endif;
								?>
							</td>
