...
<div class="col-md-10 col-sm-9 pad0" id="page_environment">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_environment tabpane_ctl_page active" id="queries">
			<p class="blog_head">Environment</p>
            <span class="posts">Posts</span>
			<ul class="list-inline btn-list_ul">
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'environment', 'act' => 'index')); ?>"><button class="btn-custom-apply btn-categories">All Posts</button></a></li>
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'environment', 'act' => 'add')); ?>"><button class="btn-custom-apply btn-categories">Add Environment</button></a></li>
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'environment', 'act' => 'categories')); ?>"><button class="btn-custom-apply btn-categories">Categories</button></a></li>
			</ul>
				...
				<ul class="list-inline list_all">
					<li>
						<select class="select_list" id="select_list_environment">
							...
							<option value="delete_environment" act="deleteEnvironmentArticle">Delete</option>
							<option value="activate_environment" act="changeStatusManyEnvironmentArticle">Activate</option>
							<option value="deactivate_environment" act="changeStatusManyEnvironmentArticle">Deactivate</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_apply_environment_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_environment_author">
						...
						<select class="select_list" id="select_list_environment_category">
						...
						<select class="select_list" id="select_list_environment_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_environment_table'>Filter</button></li>
					</li>
				</ul>
			</div>
			<div class="col-md-6 col-sm-12">
				<form id="form-environment-search">
			...
			<table class="table table-striped dataTable" id="blog_table" controller="environment" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
				<tr>
					...
					<?php if(count($this->records['data'])) { ?>
						...
						<tr role="row" id="itemID_<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								<div class="checkbox">
									...
									<label for="<?php echo("checkbox-".$record['id']);?>">
										<a target="_blank" href="<?php echo RootURL."environment/view/".$record['slug'] ?>" id="viewUser<?=$record['id'];?>">
								...
								<ul class="list-inline">
									<li>
										<a target="_blank" class='btn-delete-table' href="<?php echo RootURL."environment/view/".$record['slug'] ?>">
											View
										</a>	
									</li>
									<li>
										<a class='btn-delete-table' href="<?php echo (vendor_app_util::url(["ctl"=>"environment", "act"=>"edit/".$record['id']])) ?>" id="<?php echo("edit".$record['id']);?>" type="button" >Edit</a>
									</li>
									<li>
										<button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteEnvironmentArticle">Delete</button>
									</li>
								</ul>
							</td>
							...
							<td class="webShow" id="<?php echo("featured_image".$record['id']);?>">
								<p class="andrew">
									<a target="_blank" href="<?php echo RootURL."environment/view/".$record['slug'] ?>" id="featured_imageViewUser<?=$record['id'];?>">
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								...
								?>
								<button type="button" class="btn btn-primary change_status_environment" alt="<?php echo($record['id']);?>,0,changeStatusEnvironmentArticle" >Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_environment" alt="<?php echo($record['id']);?>,1,changeStatusEnvironmentArticle">Inactive</button>
								<?php 
									endif;
								?>
							</td>
