...
<div class="col-md-10 col-sm-9 pad0" id="page_must_reads">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
				...
				<ul class="list-inline list_all">
					<li>
						<select class="select_list" id="select_list_must_reads">
							...
							<option value="delete_must_reads" act="deleteMustReadsCategory">Delete</option>
							<option value="activate_must_reads" act="changeStatusManyMustReadsCategory">Activate</option>
							<option value="deactivate_must_reads" act="changeStatusManyMustReadsCategory">Deactivate</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_apply_must_reads_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_must_reads_categories_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_must_reads_categories_table'>Filter</button></li>
					</li>
					<li>
                        <form id="form-must_reads-categories-add">
                            <label>
			...
			<div class="col-md-6 col-sm-12">
				<form id="form-must_reads-categories-search">
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								...
								<ul class="list-inline features">
									<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-must_reads-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteMustReadsCategory">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								<?php 
									if($record['status'] == 1 ):
								?>
								<button type="button" class="btn btn-primary change_status_must_reads" alt="<?php echo($record['id']);?>,0,changeStatusMustReadsCategory">Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_must_reads" alt="<?php echo($record['id']);?>,1,changeStatusMustReadsCategory">Inactive</button>
								<?php 
									endif;
								?>
							</td>
