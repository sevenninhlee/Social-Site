...
<div class="col-md-10 col-sm-9 pad0" id="page_news">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">News categories</p>
				...
				<ul class="list-inline list_all">
					<li>
						<select class="select_list" id="select_list_news">
							...
							<option value="delete_news" act="deleteNewCategory">Delete</option>
							<option value="activate_news" act="changeStatusManyNewCategory">Activate</option>
							<option value="deactivate_news" act="changeStatusManyNewCategory">Deactivate</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_apply_news_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_news_categories_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_news_categories_table'>Filter</button></li>
					</li>
					<li>
                        <form id="form-news-categories-add">
                            <label>
			...
			<div class="col-md-6 col-sm-12">
				<form id="form-news-categories-search">
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								...
								<ul class="list-inline features">
									<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-news-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteNewCategory">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								<?php 
									if($record['status'] == 1 ):
								?>
								<button type="button" class="btn btn-primary change_status_news" alt="<?php echo($record['id']);?>,0,changeStatusNewCategory">Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_news" alt="<?php echo($record['id']);?>,1,changeStatusNewCategory">Inactive</button>
								<?php 
									endif;
								?>
							</td>
