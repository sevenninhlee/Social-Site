...
<div class="col-md-10 col-sm-9 pad0" id="page_news">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_news tabpane_ctl_page active" id="queries">
			<p class="blog_head">News</p>
				...
				<ul class="list-inline list_all">
					<li>
						<select class="select_list" id="select_list_news">
							...
							<option value="delete_news" act="deleteNewArticle">Delete</option>
							<option value="activate_news" act="changeStatusManyNewArticle">Activate</option>
							<option value="deactivate_news" act="changeStatusManyNewArticle">Deactivate</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_apply_news_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_news_author">
						...
						<select class="select_list" id="select_list_news_category">
						...
						<select class="select_list" id="select_list_news_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_news_table'>Filter</button></li>
					</li>
				</ul>
			</div>
			<div class="col-md-6 col-sm-12">
				<form id="form-news-search">
					...
					<?php if(count($this->records['data'])) { ?>
						...
						<tr role="row" id="itemID_<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								<div class="checkbox">
									...
									<label for="<?php echo("checkbox-".$record['id']);?>">
										<a target="_blank" href="<?php echo RootURL."news/view/".$record['slug'] ?>" id="viewUser<?=$record['id'];?>">
								...
								<ul class="list-inline">
									<li>
										<a target="_blank" class='btn-delete-table' href="<?php echo RootURL."news/view/".$record['slug'] ?>">
											View
										</a>	
									</li>
									<li>
										<a class='btn-delete-table' href="<?php echo (vendor_app_util::url(["ctl"=>"news", "act"=>"edit/".$record['id']])) ?>" id="<?php echo("edit".$record['id']);?>" type="button" >Edit</a>
									</li>
									<li>
										<button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteNewArticle">Delete</button>
									</li>
								</ul>
							</td>
							...
							<td class="webShow" id="<?php echo("featured_image".$record['id']);?>">
								<p class="andrew">
									<a target="_blank" href="<?php echo RootURL."news/view/".$record['slug'] ?>" id="featured_imageViewUser<?=$record['id'];?>">
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								...
								?>
								<button type="button" class="btn btn-primary change_status_news" alt="<?php echo($record['id']);?>,0,changeStatusNewArticle" >Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_news" alt="<?php echo($record['id']);?>,1,changeStatusNewArticle">Inactive</button>
								<?php 
									endif;
								?>
							</td>
