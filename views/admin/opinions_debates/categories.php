...
<div class="col-md-10 col-sm-9 pad0">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">Opinions Debates categories</p>
				...
				<ul class="list-inline list_all">
					<li>
                        <select class="select_list" id="select_list_opinions_debates">
							...
							<option value="delete_opinions_debates" act="deleteOpinionsDebatesCategory">Delete</option>
							<option value="activate_opinions_debates" act="changeStatusManyOpinionsDebatesCategory">Activate</option>
							<option value="deactivate_opinions_debates" act="changeStatusManyOpinionsDebatesCategory">Deactivate</option>
                        </select>
						<button class="btn-custom btn btn-apply" id='btn_apply_opinions_debates_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_opinions_debates_categories_status">
							...
							<option value="disable" <?php if(isset($app['prs']['status']) && $app['prs']['status']=='disable') echo 'selected' ?>>Inactive</option>
						</select>
						<button class="btn-custom btn btn-apply" id='btn_filter_opinions_debates_categories_table'>Filter</button></li>
					</li>
					<li>
                        <form id="form-opinions_debates-categories-add">
                            <label>
								<input style="width:95%;" type="text" class="add txt_box txt_box_opinion_debate_category" name='add' placeholder="" aria-controls="example" id='add'>
							</label>
                            <button type="submit" class="btn-custom btn btn-apply">Add category</button>
                        </form>
                    </li>
					<li>
						<p id='form-opinion_debate-categories-err-name' class='text-danger text-center'></p>
					</li>
				</ul>
			</div>
			<div class="col-md-6 col-sm-12">
				<form id="form-opinions_debates-categories-search">
			...
			<table class="table table-striped dataTable" id="opinion_debate_table" controller="opinions_debates" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
				<tr>
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								...
								<ul class="list-inline features">
									<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-opinions_debates-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteOpinionsDebatesCategory">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								<?php 
									if($record['status'] == 1 ):
								?>
								<button type="button" class="btn btn-primary change_status_opinions_debates" alt="<?php echo($record['id']);?>,0,changeStatusOpinionsDebatesCategory">Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_opinions_debates" alt="<?php echo($record['id']);?>,1,changeStatusOpinionsDebatesCategory">Inactive</button>
								<?php 
									endif;
								?>
							</td>
