...
<div class="col-md-10 col-sm-9 pad0">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">Queries categories</p>
				...
				<ul class="list-inline list_all">
					<li>
                        <select class="select_list" id="select_list_queries">
							...
							<option value="delete_queries" act="deleteQueryCategory">Delete</option>
							<option value="activate_queries" act="changeStatusManyQueryCategory">Activate</option>
							<option value="deactivate_queries" act="changeStatusManyQueryCategory">Deactivate</option>
                        </select>
						<button class="btn-custom btn btn-apply" id='btn_apply_queries_table'>Apply</button>
					</li>
					<li>
						<select class="select_list" id="select_list_queries_categories_status">
						...
						<button class="btn-custom btn btn-apply" id='btn_filter_queries_categories_table'>Filter</button></li>
					</li>
					<li>
                        <form id="form-queries-categories-add">
                            <label>
					...
					<li>
						<p id='form-queries-categories-err-name' class='text-danger text-center'></p>
					</li>
				</ul>
			</div>
			<div class="col-md-6 col-sm-12">
				<form id="form-queries-categories-search">
			...
			<table class="table table-striped dataTable" id="queries_table" controller="queries" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
				<tr>
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act">
								...
								<ul class="list-inline features">
									<li><button id="edit-<?php echo $record['id']; ?>" type="button" class="btn-delete-table edit-record-queries-category" alt="<?php echo $record['id']; ?>">Edit</button></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteQueryCategory">Delete</button></li>
								</ul>
							</td>
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								<?php 
									if($record['status'] == 1 ):
								?>
								<button type="button" class="btn btn-primary change_status_queries" alt="<?php echo($record['id']);?>,0,changeStatusQueryCategory">Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_queries" alt="<?php echo($record['id']);?>,1,changeStatusQueryCategory">Inactive</button>
								<?php 
									endif;
								?>
							</td>
