...
<div class="col-md-10 col-sm-9 pad0">
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane tabpane_blogs active" id="queries">
			<p class="blog_head">Queries</p>
            <span class="posts">Posts</span>
			<ul class="list-inline btn-list_ul">
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'queries', 'act' => 'index')); ?>"><button class="btn-custom-apply btn-categories">All Posts</button></a></li>
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'queries', 'act' => 'add')); ?>"><button class="btn-custom-apply btn-categories">Add New</button></a></li>
				<li><a href="<?=vendor_app_util::url(array('ctl'=>'queries', 'act' => 'categories')); ?>"><button class="btn-custom-apply btn-categories">Categories</button></a></li>
			</ul>
			...
			<div class="row">
				<div class="col-md-6 col-sm-12">
					...
					<ul class="list-inline list_all">
						<li>
	                        <select class="select_list" id="select_list_queries">
								...
								<option value="delete_queries" act="deleteQueryArticle">Delete</option>
								<option value="activate_queries" act="changeStatusManyQueryArticle">Activate</option>
								<option value="deactivate_queries" act="changeStatusManyQueryArticle">Deactivate</option>
	                        </select>
					...
					<ul class="list-inline list_all">
						<li>
							<select class="select_list" id="select_list_queries_author">
							...
							<select class="select_list" id="select_list_queries_category">
							...
							<select class="select_list" id="select_list_queries_status">
							...
							<button class="btn-custom btn btn-apply" id='btn_filter_queries_table'>Filter</button></li>
						</li>
					</ul>
				</div>
				<div class="col-md-6 col-sm-12">
					<form id="form-queries-search">
			...
			<table class="table dataTable" id="queries_table" controller="queries" role="grid" aria-describedby="example1_info">
				<tr>
					...
					<?php if(count($this->records['data'])) { ?>
						...
						<tr role="row" id="row<?=$record['id'];?>">
							...
							<td id="<?php echo("checkbox".$record['id']);?>" class="checkboxRecord btn-act" style='max-width:250px;'>
								<div class="checkbox">
									...
									<label for="<?php echo("checkbox-".$record['id']);?>">
										<a target="_blank" href="<?php echo RootURL."queries/view/".$record['slug'] ?>" id="viewUser<?=$record['id'];?>">
								...
								<ul class="list-inline">
									<li>
										<a target="_blank" class='btn-delete-table' href="<?php echo RootURL."queries/view/".$record['slug'] ?>" id="viewUser<?=$record['id'];?>">
											View
										</a>	
									</li>
									<li><a class='btn-delete-table' href="<?php echo (vendor_app_util::url(["ctl"=>"queries", "act"=>"edit/".$record['id']])) ?>" id="<?php echo("edit".$record['id']);?>" type="button" >Edit</a></li>
									<li><button id="delItem<?php echo $record['id']; ?>" type="button" class="btn-delete-table delItem-record" alt="<?php echo $record['id']; ?>,deleteQueryArticle">Delete</button></li>
								</ul>
							</td>
							...
							<td class="webShow" id="<?php echo("featured_image".$record['id']);?>">
								<p class="andrew">
									<a target="_blank" href="<?php echo RootURL."queries/view/".$record['slug'] ?>">
							...
							<td class="tabletShow" id="<?php echo("status".$record['id']);?>">
								...
								?>
								<button type="button" class="btn btn-primary change_status_queries" alt="<?php echo($record['id']);?>,0,changeStatusQueryArticle" >Active</button>
								<?php 
									else:
								?>
								<button type="button" class="btn btn-warning change_status_queries" alt="<?php echo($record['id']);?>,1,changeStatusQueryArticle">Inactive</button>
								<?php 
									endif;
								?>
							</td>
